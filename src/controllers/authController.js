...
export const AuthController = {
  register: async (req, res) => {
    ...
    await Activity.create({ type: 'user_registered', message: `New user registered: ${user.name}`, user: user._id });

    const payload = { sub: user._id.toString(), role: user.role };
    const jti = uuidv4();
    const access = signAccessToken(payload, process.env.JWT_ACCESS_SECRET, process.env.ACCESS_TOKEN_EXPIRES || '15m');
    const refresh = signRefreshToken(payload, process.env.JWT_REFRESH_SECRET, process.env.REFRESH_TOKEN_EXPIRES || '7d', jti);
  ...
  logout: async (req, res) => {
    try {
      ...
      const isProd = process.env.NODE_ENV === 'production';
      const common = { httpOnly: true, sameSite: 'lax', secure: isProd, path: '/', maxAge: 0 };
  ...
  refresh: async (req, res) => {
    ...
    try {
      ...
      const tokenDoc = await RefreshToken.findOne({ jti: decoded.jti, revoked: false });
      if (!tokenDoc) return res.status(401).json({ success: false, error: 'Invalid refresh token' });
      if (tokenDoc.expiresAt < new Date()) return res.status(401).json({ success: false, error: 'Refresh token expired' });
  ...
  me: async (req, res) => {
    ...
    res.json({ success: true, data: { id: user._id, name: user.name, email: user.email, role: user.role, status: user.status, preferences: user.preferences, joinedAt: user.joinedAt, lastActiveAt: user.lastActiveAt } });
  },
};